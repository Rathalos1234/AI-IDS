...
class NetworkMonitor:
    ...
    def __init__(self, config) -> None:
        ...
        contamination = self.config.getfloat('IsolationForest', 'Contamination', fallback=0.05)
        n_estimators = self.config.getint('IsolationForest', 'NEstimators', fallback=200)
        random_state = self.config.getint('IsolationForest', 'RandomState', fallback=42)
        self.detector = AnomalyDetector(contamination=contamination, n_estimators=n_estimators, random_state=random_state)

        self.logger = logging.getLogger("ids.monitor")
        self.logger.setLevel(self._parse_log_level(self.config.get('Logging', 'LogLevel', fallback='INFO')))
        self._ensure_handlers()

        self.online_retrain_interval = self.config.getint('Monitoring', 'OnlineRetrainInterval', fallback=0)
    ...
    def _ensure_handlers(self) -> None:
        if not self.logger.handlers:
            ...
            if self.config.getboolean('Logging', 'EnableFileLogging', fallback=True):
                ...
                fh = logging.FileHandler(os.path.join(log_dir, f"{self.config.get('Logging','AnomalyLogPrefix',fallback='anomalies')}.log"))
    ...
    def _validate_interface(self, interface: str) -> None:
        ...
        if netifaces is None:
            self.logger.warning("netifaces not installed; skipping interface validation.")
            return
        interfaces = set(netifaces.interfaces())
        if interface not in interfaces:
            raise ValueError(f"Interface '{interface}' not found. Available: {sorted(interfaces)}")

    def capture_and_train(self, interface: str, packet_count: int, model_path: str) -> None:
        ...
        self.logger.info(f"Capturing {packet_count} packets on '{interface}' for training...")
        sniff(iface=interface, prn=self.processor.process_packet, count=packet_count, store=0)
    ...
    def start_monitoring(self, interface: str, model_path: str) -> None:
        ...
        self.logger.info(f"Starting live monitoring on '{interface}'. Press Ctrl+C to stop.")
        try:
    ...
    def _analyze_packet(self, packet) -> None:
        ...
        try:
            ...
            if pred == 'Anomaly':
                ...
                msg = (f"ANOMALY: ts={last_row['timestamp']:.6f} "
                       f"{last_row['src_ip']} -> {last_row['dest_ip']} "
                       f"proto={int(last_row['protocol'])} size={int(last_row['packet_size'])} "
                       f"sport={int(last_row['sport'])} dport={int(last_row['dport'])}")
                self.logger.warning(msg)
                print("\n--- ANOMALY DETECTED ---\n" + msg + "\n------------------------\n")
            if self.online_retrain_interval > 0 and (self._packet_counter % self.online_retrain_interval == 0):
                if len(window_df) >= 50:
                    ...
                    if not win_features.empty:
                        ...
                        model_path = self.config.get('DEFAULT', 'ModelPath', fallback='models/iforest.joblib')
                        self.detector.save_model(model_path)
