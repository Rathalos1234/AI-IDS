...
def _firewall_apply(action: str, ip: str, reason: str = "") -> dict:
    info = {"applied": False, "error": None}
    try:
        if action == "block":
...
def _resolve_token(token: Optional[str]) -> tuple[Optional[str], Optional[datetime], Optional[str]]:
    if not token:
